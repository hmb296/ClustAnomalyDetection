...
####### CLUSTER STRUCTURE EVALUATION #######
if(plots){
  # visualising clustering structure using VAT:
  p1 <- ggVAT(dist(huang_matrix)) + labs(title = "VAT - Huang")
  p2 <- ggVAT(gower_matrix) + labs(title = "VAT - Gower")
  p3 <- ggVAT(dist(dafi_gower_matrix)) + labs(title = "VAT - DAFI Gower")
  
  # visualising clustering structure using iVAT:
  p4 <- ggiVAT(dist(huang_matrix)) + labs(title = "iVAT - Hunag")
  p5 <- ggiVAT(gower_matrix) + labs(title = "iVAT - Gower")
  p6 <- ggiVAT(dist(dafi_gower_matrix)) + labs(title = "iVAT - DAFI Gower")
  
  # showing plots in a grid
  grid.arrange(p1, p2, p3, p4, p5, p6, nrow = 2, ncol = 3, 
               top = paste(dataset_name, "Cluster Plots"))
}
###### TUNING k ######
source("tune_k_silhouette.R")
optimal_k_kamila <- tune_k_sil(method = "kamila", distance_matrix=gower_matrix, 
                               numeric_data = numeric_data, 
                               categorical_data = categorical_data, 
                               k_range = 2:10, plot=FALSE)
cat("Optimal k for KAMILA:", optimal_k_kamila, "\n")

optimal_k_kproto <- tune_k_sil(method = "kproto", distance_matrix=gower_matrix, 
                               data=data, k_range = 2:10, plot=FALSE)
cat("Optimal k for K-Prototypes:", optimal_k_kproto, "\n")

optimal_k_pam <- tune_k_sil(method = "pam", distance_matrix=gower_matrix, 
                            k_range = 2:10, plot=FALSE)
cat("Optimal k for PAM:", optimal_k_pam, "\n")

###### CLUSTERING ######
# k-Prototypes:
kproto_res <- kproto(data, k = optimal_k_kproto, verbose = FALSE, type="gower")
data$cluster_kproto <- kproto_res$cluster

# PAM with Gower distance:
pam_gower <- pam(gower_matrix, k = optimal_k_pam)
data$cluster_pam_gower <- pam_gower$clustering

# PAM with DAFI-Gower distance:
pam_dafi <- pam(dafi_gower_matrix, k = optimal_k_pam)
data$cluster_pam_dafi_gower <- pam_dafi$clustering

# KAMILA:
kamila_result <- kamila(numeric_data, categorical_data, numClust = optimal_k_kamila, 
                        numInit = 10)
data$cluster_kamila <- kamila_result$finalMemb
...
cluster_vars <- list(
  cluster_kproto = data$cluster_kproto,
  cluster_pam_gower = data$cluster_pam_gower,
  cluster_pam_dafi_gower = data$cluster_pam_dafi_gower,
  cluster_kamila = data$cluster_kamila
)
